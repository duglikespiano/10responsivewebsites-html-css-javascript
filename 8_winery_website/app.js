...
const pageController = () => {
	...
	if (counter1 === -1) {
		Array.from(sections).forEach((section) => {
			if (section.classList[0] === 'section5') {
				return;
			}
...
window.addEventListener('wheel', (e) => {
	...
	if (bool) {
		...
		document.querySelector(`.section${deltaY ? counter1 + 1 : counter2 + 1}-wrapper`).style.transform = `scale(${
			deltaY ? '1' : '1.5'
		})`;
	}
});
...
document.querySelector('.grapes-img').addEventListener('mouseover', () => {
	document.querySelector('.section3-wrapper').style.opacity = '1';
});

document.querySelector('.grapes-img').addEventListener('mouseout', () => {
	document.querySelector('.section3-wrapper').style.opacity = '0.5';
});
